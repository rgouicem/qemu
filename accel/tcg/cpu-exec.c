...
static inline int is_bb_shared(target_ulong pc)
{
    /* /\* fprintf(stderr, "%s:%d: checking " TARGET_FMT_lx "...", *\/ */
    /* /\*         __func__, __LINE__, pc); *\/ */
    for (struct bb_list_fenced *item = bb_list[pc % BBLIST_SIZE];
         item; item = item->next) {
        if (item->addr == pc) {
                                        ...
                                        int tb_exit, uint32_t cflags)
{
    ...
    fprintf(stderr, "%s:%d: pc=" TARGET_FMT_lx " cflags[CF_PARALLEL]=%x flags[CF_PARALLEL]=%x\n",
            __func__, __LINE__, pc, cflags & CF_PARALLEL, flags & CF_PARALLEL);
